...
object Utils {
    ...
    fun backupPath(context: Context?): String {
        ...
        return try {
            context.getExternalFilesDir(Constants.DIR_BACKUPS)?.absolutePath
                ?: context.getDir(Constants.DIR_BACKUPS, 0).absolutePath
        } catch (e: Exception) {
            Log.e(Constants.TAG, "Failed to get backup path", e)
            ""
        }
    }
    ...
    fun isValidSubUrl(value: String?): Boolean {
        ...
        try {
            ...
            if (URLUtil.isHttpUrl(value)) {
                if (value.contains(Constants.LOOPBACK)) return true

                //Check private ip address
                val uri = URI(fixIllegalUrl(value))
                if (isIpAddress(uri.host)) {
                    Constants.PRIVATE_IP_LIST.forEach {
                        if (isIpInCidr(uri.host, it)) return true
                    }
                }
            }
        } catch (e: Exception) {
            Log.e(Constants.TAG, "Failed to validate subscription URL", e)
        }
        return false
    }
