...
class VlessConverterTest {
    ...
    fun setup() {
        outbound = OutboundBean(
            protocol = ConfigType.VLESS.name,
        ...
        every { provider.createInitOutbound(ConfigType.VLESS) } returns outbound
        ...
        converter = VlessConverter(lazyProvider)
    }
    ...
    @Test
    fun `toOutbound maps address, port and user fields then applies transport and tls`() {
        val profile = ConfigProfileItem.create(ConfigType.VLESS).copy(
            server = "1.2.3.4",
            serverPort = "443",
            password = "uuid-123",
            method = "none",
        ...
        assertThat(vnext.address).isEqualTo("1.2.3.4")
        assertThat(vnext.port).isEqualTo(443)
        assertThat(vnext.users[0].id).isEqualTo("uuid-123")
        assertThat(vnext.users[0].encryption).isEqualTo("none")
        assertThat(vnext.users[0].flow).isEqualTo("xtls-rprx-vision")

        verifySequence {
            provider.createInitOutbound(ConfigType.VLESS)
    ...
    fun `toOutbound returns null when provider returns null`() {
        val nullProvider = mockk<XrayConfigProvider>(relaxUnitFun = true) {
            every { createInitOutbound(ConfigType.VLESS) } returns null
        }
        val nullLazy = object : Lazy<XrayConfigProvider> { override fun get() = nullProvider }
        val nullConverter = VlessConverter(nullLazy)
        ...
        verify(exactly = 1) { nullProvider.createInitOutbound(ConfigType.VLESS) }
